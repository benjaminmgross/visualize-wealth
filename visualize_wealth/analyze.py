...
"""
.. module:: analyze.py
   :synopsis: Contains all of the necessary functions to perform statistical
   analysis on a portfolio and its comparative benchmark
...
def value_at_risk(series, freq = 'weekly', percentile = 5.):
    ...
    **USAGE:**::
    ...
    series_rets = series[ind].apply(numpy.log).diff()
...
def rolling_ui(series, window = 21):
    ...
    **USAGE:**::
    ...
    """
    rui = pandas.Series(numpy.tile(numpy.nan, [len(series),]), index = series.index, name = 'rolling UI')
    j = 0
...
def sharpe_ratio(series, rfr = 0., freq = 'daily'):
    ...
    **RETURN:**
    ...
    fac = _interval_to_factor(freq)
    series_rets = series.apply(numpy.log).diff()
    return (numpy.exp(series_rets.mean()*fac)-1 - rfr) / (series_rets.std()*numpy.sqrt(fac))
...
def risk_adjusted_excess_returns(series, benchmark, rfr = 0., freq = 'daily'):
    ...
    **RETURNS:**
    ...
    series_rets = series.apply(numpy.log).diff()
    series_vol = series_rets.std()*numpy.sqrt(fac)

    return numpy.mean(series_rets)*fac - series_vol * benchmark_sharpe
...
def jensens_alpha(series, benchmark, rfr = 0., freq = 'daily'):
    ...
    **RETURNS:**
    ...
    return (series_rets.mean() - (rfr + beta(series, benchmark)*(
        bench_rets.mean() - rfr)))*fac
...
def tracking_error(series, benchmark, freq = 'daily'):
    ...
    **RETURNS:**
    ...
    series_rets = series.apply(numpy.log).diff()
    bench_rets = benchmark.apply(numpy.log).diff()
    return numpy.subtract(series_rets, bench_rets).std()*numpy.sqrt(fac)
...
def idiosyncratic_risk(series, benchmark, freq = 'daily'):
    ...
    **RETURNS:**
    ...
    fac = _interval_to_factor(freq)
    series_rets = series.apply(numpy.log).diff()
    bench_rets = benchmark.apply(numpylog).diff()
    series_vol = series_rets.std()*numpy.sqrt(fac)
    benchmark_vol = bench_rets.std()*numpy.sqrt(fac)
...
def idiosyncratic_as_proportion(series, benchmark, freq = 'daily'):
    ...
    **RETURNS:**
    ...
    return idiosyncratic_risk(series, benchmark, freq)**2 / (
        series_rets.std()*numpy.sqrt(fac))
...
def systematic_as_proportion(series, benchmark, freq = 'daily'):
    ...
    **RETURNS:**
    ...
    """
    fac = _interval_to_factor(freq)
    return systematic_risk(series, benchmark, freq) **2 / ((series.std() * numpy.sqrt(252)) ** 2)
...
def return_by_year(series):
    ...
    **RETURNS:**
    ...
    return pandas.Series(yearly_rets[1:].values, index = years[1:], name = series.name)
...
def generate_all_metrics(series, benchmark, freq = 'daily', rfr = 0.):
   ...
   **RETURNS:**
    ...
    if isinstance(series, pandas.Series):
        return  _gen_function(series = series, benchmark = benchmark, freq = freq, rfr = rfr)
...
def _bool_interval_index(pandas_index, interval = 'monthly'):
    ...
    boolean index to be passed visa vie DataFrame.ix[boo_index, :]
    """
    ...
    time_dict = {'weekly':weekly, 'monthly': monthly, 'quarterly': qt, 'yearly': yearly, 'ldom':ldom, 'fdom':fdom}
